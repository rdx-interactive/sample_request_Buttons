...
class SAW_Sample_Product_Grid_Elementor_Widget extends \Elementor\Widget_Base {
	...
	protected function register_controls() {
		...
		$this->add_control(
			'products_per_page',
			array(
				'label'   => __( 'Products to Show Initially', 'sample-available-for-woocommerce' ),
		...
		$this->add_control(
			'show_load_more',
			array(
				'label'        => __( 'Enable Load More', 'sample-available-for-woocommerce' ),
				'type'         => \Elementor\Controls_Manager::SWITCHER,
				'label_on'     => __( 'Yes', 'sample-available-for-woocommerce' ),
				'label_off'    => __( 'No', 'sample-available-for-woocommerce' ),
				'return_value' => 'yes',
				'default'      => 'yes',
			)
		);
		...
		$this->add_control(
			...
			array(
				'label'     => __( 'Products Per Load', 'sample-available-for-woocommerce' ),
				'type'      => \Elementor\Controls_Manager::NUMBER,
				'default'   => 12,
				'min'       => 1,
				'step'      => 1,
		...
		$this->add_control(
			'load_more_text',
			array(
				'label'       => __( 'Load More Text', 'sample-available-for-woocommerce' ),
				'type'        => \Elementor\Controls_Manager::TEXT,
				'default'     => __( 'Load more', 'sample-available-for-woocommerce' ),
				'placeholder' => __( 'Load more', 'sample-available-for-woocommerce' ),
				'condition'   => array(
					'show_load_more' => 'yes',
				),
			)
		...
		$this->add_control(
			...
			array(
				'label'       => __( 'Loading Text', 'sample-available-for-woocommerce' ),
				'type'        => \Elementor\Controls_Manager::TEXT,
				'default'     => __( 'Loading...', 'sample-available-for-woocommerce' ),
				'placeholder' => __( 'Loading...', 'sample-available-for-woocommerce' ),
				'condition'   => array(
					'show_load_more' => 'yes',
				),
			)
		...
		$this->start_controls_section(
			...
			array(
				'label'     => __( 'Load More Button', 'sample-available-for-woocommerce' ),
				'tab'       => \Elementor\Controls_Manager::TAB_STYLE,
				'condition' => array(
					'show_load_more' => 'yes',
				),
		...
		$this->add_group_control(
			...
			array(
				...
				'selector' => '{{WRAPPER}} .saw-grid-load-more-button',
			)
		);
		...
		$this->add_responsive_control(
			...
			array(
				...
				'size_units' => array( 'px', 'em', '%' ),
				'selectors'  => array(
					'{{WRAPPER}} .saw-grid-load-more-wrap' => 'margin: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
				),
			)
		);
		...
		$this->add_responsive_control(
			...
			array(
				...
				'size_units' => array( 'px', 'em', '%' ),
				'selectors'  => array(
					'{{WRAPPER}} .saw-grid-load-more-button' => 'padding: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
				),
			)
		);
		...
		$this->add_responsive_control(
			...
			array(
				...
				'selectors'  => array(
					'{{WRAPPER}} .saw-grid-load-more-button' => 'width: {{SIZE}}{{UNIT}};',
				),
			)
		);
		...
		$this->add_control(
			...
			array(
				...
				'type'      => \Elementor\Controls_Manager::COLOR,
				'selectors' => array(
					'{{WRAPPER}} .saw-grid-load-more-button' => 'color: {{VALUE}};',
				),
			)
		);
		...
		$this->add_control(
			...
			array(
				...
				'type'      => \Elementor\Controls_Manager::COLOR,
				'selectors' => array(
					'{{WRAPPER}} .saw-grid-load-more-button' => 'background-color: {{VALUE}};',
				),
			)
		);
		...
		$this->add_responsive_control(
			...
			array(
				...
				'size_units' => array( 'px', '%', 'em' ),
				'selectors'  => array(
					'{{WRAPPER}} .saw-grid-load-more-button' => 'border-radius: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
				),
			)
		);
		...
		$this->add_control(
			...
			array(
				...
				'type'      => \Elementor\Controls_Manager::COLOR,
				'selectors' => array(
					'{{WRAPPER}} .saw-grid-load-more-button:hover, {{WRAPPER}} .saw-grid-load-more-button:focus' => 'color: {{VALUE}};',
				),
			)
		);
		...
		$this->add_control(
			...
			array(
				...
				'type'      => \Elementor\Controls_Manager::COLOR,
				'selectors' => array(
					'{{WRAPPER}} .saw-grid-load-more-button:hover, {{WRAPPER}} .saw-grid-load-more-button:focus' => 'background-color: {{VALUE}};',
				),
			)
		);
		...
		$this->add_control(
			...
			array(
				...
				'selectors' => array(
					'{{WRAPPER}} .saw-grid-load-more-button:hover, {{WRAPPER}} .saw-grid-load-more-button:focus' => 'border-color: {{VALUE}};',
				),
			)
		);
	...
	protected function render() {
		...
		$initial_count = $this->get_initial_count( $settings );
		$batch_size    = $this->get_load_more_batch_size( $settings );
		$show_load_more = ! empty( $settings['show_load_more'] ) && 'yes' === $settings['show_load_more'];
		$item_index    = 0;

		echo '<div class="saw-sample-product-grid-wrap" data-saw-grid-initial="' . esc_attr( $initial_count ) . '" data-saw-grid-batch="' . esc_attr( $batch_size ) . '">';
		...
		while ( $query->have_posts() ) {
			...
			$item_index++;
			$this->render_product_card( $product, $settings, $show_load_more && $item_index > $initial_count );
		}
		...
		if ( $show_load_more && $item_index > $initial_count ) {
			$this->render_load_more_button( $settings );
		}
	 ...
	 * @param array      $settings Widget settings.
	 * @param bool       $hidden   Whether to hide the card until Load More is clicked.
	 * @return void
	 */
	private function render_product_card( $product, $settings, $hidden = false ) {
		$product_id = absint( $product->get_id() );
		$permalink  = get_permalink( $product_id );
		$label      = ! empty( $settings['button_text'] ) ? $settings['button_text'] : __( 'Request a Sample', 'sample-available-for-woocommerce' );
		$classes    = array( 'saw-sample-product-card' );

		if ( $hidden ) {
			$classes[] = 'saw-grid-product-hidden';
		}
		?>
		<article class="<?php echo esc_attr( implode( ' ', $classes ) ); ?>"<?php echo $hidden ? ' hidden' : ''; ?>>
	...
	/**
	 * Render the Load More control.
	 *
	 * @param array $settings Widget settings.
	 * @return void
	 */
	private function render_load_more_button( $settings ) {
		$label         = ! empty( $settings['load_more_text'] ) ? $settings['load_more_text'] : __( 'Load more', 'sample-available-for-woocommerce' );
		$loading_label = ! empty( $settings['load_more_loading_text'] ) ? $settings['load_more_loading_text'] : __( 'Loading...', 'sample-available-for-woocommerce' );
		?>
		<div class="saw-grid-load-more-wrap">
			<button type="button" class="saw-grid-load-more-button elementor-button" data-saw-loading-text="<?php echo esc_attr( $loading_label ); ?>">
	...
	/**
	 * Get the number of products shown before Load More.
	 *
	 * @param array $settings Widget settings.
	 * @return int
	 */
	private function get_initial_count( $settings ) {
		if ( empty( $settings['show_load_more'] ) || 'yes' !== $settings['show_load_more'] ) {
			return PHP_INT_MAX;
		}

		return ! empty( $settings['products_per_page'] ) ? max( 1, absint( $settings['products_per_page'] ) ) : 12;
	}

	/**
	 * Get the number of products revealed per Load More click.
	 *
	 * @param array $settings Widget settings.
	 * @return int
	 */
	private function get_load_more_batch_size( $settings ) {
		return ! empty( $settings['load_more_batch_size'] ) ? max( 1, absint( $settings['load_more_batch_size'] ) ) : $this->get_initial_count( $settings );
	}
